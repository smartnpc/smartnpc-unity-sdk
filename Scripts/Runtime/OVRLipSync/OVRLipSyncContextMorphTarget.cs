    ...
    public class OVRLipSyncContextMorphTarget : MonoBehaviour
    {
        ...
        void CheckForKeys()
        {
            if (enableVisemeTestKeys)
            {
                ...
                {
                    CheckVisemeKey(visemeTestKeys[i], i, visemeBlendRange);
                }
            }
        ...
        void SetVisemeToMorphTarget(OVRLipSync.Frame frame)
        {
            for (int i = 0; i < visemeToBlendTargets.Length; i++)
            {
                ...
                {
                    // Viseme blend weights are in range of 0->1.0, we need to make range visimeBlendRange
                    skinnedMeshRenderer.SetBlendShapeWeight(
                        visemeToBlendTargets[i],
                        frame.Visemes[i] * visemeBlendRange);
                }
            }
        }
        ...
        void SetLaughterToMorphTarget(OVRLipSync.Frame frame)
        {
            if (laughterBlendTarget != -1)
            {
                ...
                skinnedMeshRenderer.SetBlendShapeWeight(
                    laughterBlendTarget,
                    laughterScore * visemeBlendRange);
            }
        }
        ...
        void CheckLaughterKey()
        {
            ...
            {
                lipsyncContext.SetLaughterBlend(visemeBlendRange);
            }
